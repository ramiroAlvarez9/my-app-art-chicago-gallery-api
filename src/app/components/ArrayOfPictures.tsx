...
const ArrayOfPictures = ({ imagesArray }: Props) => {
  ...
  const arrayDataImages: (JSX.Element | undefined)[] = imagesArray.map(
    (object: any) => {
      ...
      if ( object.image_id != null && !object.image_id.includes("883af6d-fd95-c855-ee7e-2093348774") ) {
        
        return (
          <article className="images__container" key={object.id} >
            <Image
              key={object.id}
            ...
            />
            <h5 className={`images__container--title ${fira_sans.className}`}  key={object.id}>
              {object.title}
            </h5>
          </article>
        );
